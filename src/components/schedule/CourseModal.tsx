...
export function CourseModal({
  ...
  const [color, setColor] = useState(DOPAMINE_COLORS[0].value)
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialCourse) {
        ...
        setDay(initialCourse.day)
        setStartHour(initialCourse.startHour)
        setStartMinute(initialCourse.startMinute)
        ...
        setLocation('')
        setTeacher('')
        setColor(DOPAMINE_COLORS[Math.floor(Math.random() * DOPAMINE_COLORS.length)].value)
      }
      setErrors({})
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-md bg-card rounded-3xl shadow-2xl overflow-hidden"
        ...
        {/* Actions - iOS style bottom sheet buttons */}
